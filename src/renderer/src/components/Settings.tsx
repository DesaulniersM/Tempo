    ...
    <div className="settings-page">
      <div className="card">
        ...
        <form onSubmit={handleAdd} style={{ display: 'flex', gap: '1rem', marginBottom: '2rem' }}>
          <input type="text" placeholder="Add new project..." value={newCatName} onChange={(e) => setNewCatName(e.target.value)} style={{ flex: 1 }} />
          <input type="color" value={newCatColor} onChange={(e) => setNewCatColor(e.target.value)} style={{ width: '50px' }} />
          <button type="submit" className="btn btn-primary"><Plus size={18} /> Add Category</button>
        </form>
        <div className="categories-list">
          {categories.map(cat => (
            <div key={cat.id} className="category-item">
              {editingId === cat.id ? (
                <form onSubmit={(e) => handleUpdate(e, cat.id)} style={{ display: 'flex', gap: '0.75rem', width: '100%', alignItems: 'center' }}>
